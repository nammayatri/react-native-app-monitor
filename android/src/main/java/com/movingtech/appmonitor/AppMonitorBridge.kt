...
@ReactModule(name = AppMonitorBridge.NAME)
class AppMonitorBridge(private val reactContext: ReactApplicationContext) : NativeAppMonitorSpec(reactContext) {
    ...
    override fun addEvent(eventType: String?, eventName: String?, eventPayload: ReadableMap?) {
    ...
    override fun replaceUserId(userId: String, promise: Promise) {
        val success = appMonitor.replaceUserId(userId)
            .get() // This will block until the API call completes
